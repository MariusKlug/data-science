...
addOptional(p, 'drops_size', 10, @isnumeric)
...
drops_size = p.Results.drops_size;
...
elseif numel(y_values) ~= size(data,2)
    error('y_values must be the same length as size(data,2)!')
end
...
for i_column = 1:size(data,2)
    ...
    handles{i_column,2}.SizeData = drops_size;
